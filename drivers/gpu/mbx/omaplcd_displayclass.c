...
#ifdef BUFFER_OVERLAY
#ifdef OMAP2420
#include "omap2420_display.h"
#else	/*  */
  
#ifdef OMAP2430
#include "omap2430_display.h"
#else	/*  */
...
{
  return (OMAPLCD_DEVINFO *) gpvAnchor;
}

static IMG_VOID
SetAnchorPtr (OMAPLCD_DEVINFO * psDevInfo) 
{
  gpvAnchor = (IMG_VOID *) psDevInfo;
} 

#ifdef BUFFER_OVERLAY
  IMG_VOID FlipOverlay (OMAPLCD_DEVINFO * psDevInfo,
...
omaplcd_ddc_swaptosystem (IMG_HANDLE hPrivateDevice) 
{
  ...
  psDevInfo = (OMAPLCD_DEVINFO *) hPrivateDevice;
...
static IMG_BOOL
omaplcd_ddc_process_vsyncflip (IMG_HANDLE hCmdCookie,
			       IMG_UINT32 ui32DataSize, IMG_VOID * pvData) 
{
  OMAPLCD_PRIVATE_FLIP_CMD * psFlipCmd;
  OMAPLCD_DEVINFO * psDevInfo;
  OMAPLCD_VSYNC_FLIP_ITEM * psFlipItem;
  OMAPLCD_SWAP_CHAIN * psSwapChain;
  psFlipCmd = (OMAPLCD_PRIVATE_FLIP_CMD *) pvData;
  if (psFlipCmd == IMG_NULL || psFlipCmd->ui32CmdSize != ui32DataSize)
    
    {
      return IMG_FALSE;
    }
  psDevInfo = (OMAPLCD_DEVINFO *) psFlipCmd->hDevHandle;
...
IMG_VOID VSyncFlip (OMAPLCD_DEVINFO * psDevInfo) 
{
  ...
  ui32IntMask =
    ReadReg ((IMG_VOID *) ((IMG_UINT8 *) psDevInfo->sFBInfo.pvRegs +
			   OMAPLCD_IRQENABLE));
  WriteReg ((IMG_VOID *) ((IMG_UINT8 *) psDevInfo->sFBInfo.pvRegs +
			    OMAPLCD_IRQENABLE), OMAPLCD_INTMASK_OFF);
  if (!psDevInfo->psSwapChain)
    
    {
      WriteReg ((IMG_VOID *) ((IMG_UINT8 *) psDevInfo->sFBInfo.pvRegs +
				OMAPLCD_IRQENABLE), ui32IntMask);
      return;
    }
...
PVRSRV_ERROR InitMain (IMG_VOID * pvKernelPage) 
{
  ...
  if (psDevInfo == IMG_NULL)
    
    {
      ...
      PFN_CMD_PROC pfnCmdProcList[OMAPLCD_COMMAND_COUNT];
      IMG_UINT32 aui32SyncCountList[OMAPLCD_COMMAND_COUNT][2];
      IMG_UINT32 i;
	...
	
	{
      ...
      pfnCmdProcList[OMAPLCD_FLIP_COMMAND] = omaplcd_ddc_process_vsyncflip;
      aui32SyncCountList[OMAPLCD_FLIP_COMMAND][0] = 0;
      aui32SyncCountList[OMAPLCD_FLIP_COMMAND][1] = 2;
      if (psJTable->
		 pfnPVRSRVRegisterCmdProcList (psDevInfo->sClientDevInfo.
					       ui32DeviceID,
					       &pfnCmdProcList[0],
					       aui32SyncCountList,
					       OMAPLCD_COMMAND_COUNT) !=
		 PVRSRV_OK)
	...
	psDevInfo->sClientDevInfo.asDisplayFormatList[0];
      for (i = 0; i < OMAPLCD_MAX_BACKBUFFERS; i++)
...
PVRSRV_ERROR Init (IMG_VOID) 
{
  ...
  if (sPVRDCJTable.
	 pfnPVRSRVRegisterDisplayClassDevice (omaplcd_ddc_swaptosystem,
					      IMG_NULL,
	...
	
	{
      ...
      if (psJTable->
	     pfnPVRSRVRemoveCmdProcList (psDevInfo->sClientDevInfo.
					 ui32DeviceID,
					 OMAPLCD_COMMAND_COUNT) != PVRSRV_OK)
...
IMG_EXPORT 
  PVRSRV_ERROR GetClientInfo (OMAPLCD_CLIENT_DEVINFO * psClientDevInfo) 
{
  OMAPLCD_DEVINFO * psDevInfo;
  psDevInfo = GetAnchorPtr ();
  if (psDevInfo == IMG_NULL)
...
#if 0
static SYSTEM_ADDR *
omaplcd_ddc_mapphystosysaddr (IMG_UINT32 ui32DevPhysAddr) 
{
  return IMG_NULL;
}

static IMG_VOID
omaplcd_ddc_setmode (DISPLAY_FORMAT * psFormat, DISPLAY_DIMS * psDims,
		     SYSTEM_ADDR * psSysAddr) 
{
} static OMAPLCD_BUFFER *

omaplcd_ddc_getsystembuffer (IMG_VOID) 
{
  return IMG_NULL;
}
...
#endif	/*  */
  IMG_EXPORT 
  PVRSRV_ERROR SetMode (OMAPLCD_DEVINFO * psDevInfo,
...
void
FlushQueue (OMAPLCD_DEVINFO * psDevInfo,
	    ...
	    IMG_VOID * pArg) 
{
  ...
  PVRSRV_ERROR OEMFunc (OMAPLCD_DEVINFO * psDevInfo, IMG_UINT32 ui32CmdID,
			 ...
			 IMG_UINT32 ui32pbOutDataSize) 
{
  ...
  switch (ui32CmdID)
	...
	WriteReg ((IMG_VOID *) ((IMG_UINT8 *) psDevInfo->sFBInfo.pvRegs +
				   OMAPLCD_CONTROL),
		     ui32Control | OMAP_CONTROL_LCDENABLE |
		     OMAP_CONTROL_GOLCD);
	while (ReadReg
		 ((IMG_VOID *) ((IMG_UINT8 *) psDevInfo->sFBInfo.pvRegs +
				OMAPLCD_CONTROL)) & OMAP_CONTROL_GOLCD);
	eError = PVRSRV_OK;
      }
      break;
...
#endif	/*  */
{
  ...
  IMG_UINT32 i;
	...
	
	{
      ...
      psSwapChain = AllocKernelMem (sizeof (OMAPLCD_SWAP_CHAIN));
...
IMG_EXPORT 
  PVRSRV_ERROR DestroySwapchain (OMAPLCD_SWAP_CHAIN * psSwapChain,
				 IMG_BOOL bDestroyCommandQ) 
{
  ...
  psDevInfo = (OMAPLCD_DEVINFO *) psSwapChain->hDevice;
  if (!psDevInfo->bIsPoweredDown)
