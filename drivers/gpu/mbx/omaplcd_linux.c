...
#include "omaplcdif.h"
#include "omaplcd_int.h"

#define DRVNAME "omaplcd"
...
#define COPY_TO_USER(arg, addr, size) \
	if(copy_to_user(arg, addr, size) != 0) { \
		printk("OMAPLCD: Copy to user failed."); \
	}

#define COPY_FROM_USER(arg, addr, size) \
	if(copy_from_user(arg, addr, size) != 0) { \
		printk("OMAPLCD: Copy from user failed."); \
	}

MODULE_SUPPORTED_DEVICE(DRVNAME);

#if defined(CONFIG_MACH_NOKIA_N800) || defined(CONFIG_MACH_NOKIA_RX44) || defined(CONFIG_MACH_NOKIA_RX48)
static long fbsize = 800*480*2;
#endif
...
int OMAPLCDBridgeDispatch(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg);
int OMAPLCDOpen(struct inode* pInode, struct file* pFile);
int OMAPLCDRelease(struct inode* pInode, struct file* pFile);


static int AssignedMajorNumber = 0;

static IMG_CHAR szClientDrvName[] = "libomaplcd.so";

#define unref__ __attribute__ ((unused))

static unsigned int *kmalloc_area = IMG_NULL;

static int OMAPLCDMMap(struct file *filp, struct vm_area_struct *vma)
{
	int ret = 0;
...
static struct file_operations cldc_fops = {
	owner:THIS_MODULE,
	ioctl:OMAPLCDBridgeDispatch,
	open:OMAPLCDOpen,
	release:OMAPLCDRelease,
	mmap:OMAPLCDMMap
};


int OMAPLCDOpen(struct inode unref__ * pInode, struct file unref__ * pFile)
{
	int Ret = 0;
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0))
	MOD_INC_USE_COUNT;
...
int OMAPLCDRelease(struct inode unref__ * pInode, struct file unref__ * pFile)
{
	int Ret = 0;
...
static int __init OMAPLCD_Init(void)
{
	AssignedMajorNumber =
		register_chrdev(AssignedMajorNumber, DRVNAME, &cldc_fops);

	printk ("OMAPLCD_Init: major device %d\n", AssignedMajorNumber);

	if (AssignedMajorNumber <= 0)
	{
		printk ("OMAPLCD_Init: unable to get major\n");
		return -EBUSY;
	}
...
static void __exit OMAPLCD_Cleanup(void)
{    
	if(Deinit() != PVRSRV_OK)
	{
		printk ("OMAPLCD_Cleanup: can't deinit device\n");
	}
#if 0 
...
{
	OMAPLCD_FBINFO *psFBInfo = &psDevInfo->sFBInfo;
...
{
	OMAPLCD_DEVINFO *psDevInfo = (OMAPLCD_DEVINFO *)dev_id;
...
vsync_isr(void *arg, struct pt_regs *regs)
{
	...
	OMAPLCD_DEVINFO *psDevInfo = (OMAPLCD_DEVINFO *)arg;
...
#if defined(OMAP2420)
	if(request_irq(OMAPLCD_IRQ, vsync_isr, SA_SHIRQ, "OMAPLCD Flip", psDevInfo))
	{
...
#else
#if defined(OMAP2430)
	if (omap2_disp_register_isr(vsync_isr, psDevInfo,
...
#if defined(OMAP2420)
	free_irq(OMAPLCD_IRQ, psDevInfo);
#else
#if defined(OMAP2430)
	PVR_UNREFERENCED_PARAMETER(psDevInfo);
	omap2_disp_unregister_isr(vsync_isr);
#endif
#endif
...
int OMAPLCDBridgeDispatch(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	OMAPLCD_BRIDGE_PACKAGE *psBridgePackageUM = (OMAPLCD_BRIDGE_PACKAGE *)arg;
	OMAPLCD_BRIDGE_PACKAGE sBridgePackageKM;
	...
	bAccessibleParams = access_ok(VERIFY_WRITE, psBridgePackageUM, sizeof(OMAPLCD_BRIDGE_PACKAGE));

	if (bAccessibleParams)
	{
		COPY_FROM_USER(&sBridgePackageKM, psBridgePackageUM, sizeof(OMAPLCD_BRIDGE_PACKAGE));
		...
		switch (cmd) 
		{
			case OMAPLCDIO_GET_CLIENTINFO :
			{
				OMAPLCD_GET_CLIENTINFO_OUT_DATA	sOutData;
			...
			case OMAPLCDIO_GET_DEVINFO :
			{
				OMAPLCD_GET_DEVINFO_OUT_DATA    sOutData;
			...
			case OMAPLCDIO_SET_MODE :
			{
				OMAPLCD_SET_MODE_INOUT_DATA	sInOut;
	
				COPY_FROM_USER(&sInOut, sBridgePackageKM.pvParamIn, sizeof(OMAPLCD_SET_MODE_INOUT_DATA));
	
				sInOut.eError = SetMode((OMAPLCD_DEVINFO *)sInOut.hDevInfo,
										&sInOut.sSysBusAddr,
										&sInOut.sModeInfo);
			...
			case OMAPLCDIO_CREATE_SWAPCHAIN :
			{
				OMAPLCD_CREATE_SWAP_CHAIN_INOUT_DATA	sInOut;
	
				COPY_FROM_USER(&sInOut, sBridgePackageKM.pvParamIn, sizeof(OMAPLCD_CREATE_SWAP_CHAIN_INOUT_DATA));
			...
			case OMAPLCDIO_DESTROY_SWAPCHAIN :
			{
				OMAPLCD_DESTROY_SWAP_CHAIN_INOUT_DATA	sInOut;
	
				COPY_FROM_USER(&sInOut, sBridgePackageKM.pvParamIn, sizeof(OMAPLCD_DESTROY_SWAP_CHAIN_INOUT_DATA));
	
				sInOut.eError = DestroySwapchain((OMAPLCD_SWAP_CHAIN *)sInOut.hSwapChain, IMG_TRUE);

				COPY_TO_USER(sBridgePackageKM.pvParamOut, &sInOut, sBridgePackageKM.ui32OutBufferSize);
				break;			
			}
			case OMAPLCDIO_IS_POWER_DOWN:
			{
				OMAPLCD_IS_POWER_DOWN_OUT_DATA sOut;
				sOut.bResult = IsPowerDown((OMAPLCD_DEVINFO*)GetAnchorPtr());
				COPY_TO_USER(sBridgePackageKM.pvParamOut, &sOut, sBridgePackageKM.ui32OutBufferSize);
				break;
			}
			case OMAPLCDIO_INIT_MAIN:
			{
				OMAPLCD_INIT_MAIN_IN_DATA sIn;
				COPY_FROM_USER(&sIn, sBridgePackageKM.pvParamIn, sizeof(OMAPLCD_INIT_MAIN_IN_DATA));

				InitMain((IMG_VOID*)sIn.pvKernelPageDevInfo);
				break;
			}
			case OMAPLCDIO_DEINIT_MAIN:
			{
				DeinitMain();
				break;
			}
			case OMAPLCDIO_UNINSTALL_VSYNC_ISR:
			{
				UnInstallVsyncISR(GetAnchorPtr());
				break;
			}
			case OMAPLCDIO_INSTALL_VSYNC_ISR:
			{
...
IMG_BOOL IsPowerDown(OMAPLCD_DEVINFO *psDevInfo)
{
	...
	if(ReadReg((IMG_VOID *)((IMG_UINT8 *)psDevInfo->sFBInfo.pvRegs + OMAPLCD_CONTROL)) & (OMAP_CONTROL_DIGITALENABLE | OMAP_CONTROL_LCDENABLE) )
	{
